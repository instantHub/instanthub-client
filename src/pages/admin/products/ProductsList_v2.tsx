...
import { ProductCard_v2 } from "./components";
...
const getStoredFilters = () => {
  try {
    return {
      ...
      categoryId: sessionStorage.getItem(STORAGE_KEYS.CATEGORY) || "",
      status: sessionStorage.getItem(STORAGE_KEYS.STATUS) || "",
      page: parseInt(sessionStorage.getItem(STORAGE_KEYS.PAGE) || "1", 10),
    };
  } catch (error) {
    console.error("Error reading from sessionStorage:", error);
    return { search: "", categoryId: "", status: "", page: 1 };
  }
};

const setStoredFilter = (key: string, value: string | number) => {
  try {
...
export const ProductsList_v2: React.FC = () => {
  ...
  useEffect(() => {
    setStoredFilter(STORAGE_KEYS.SEARCH, search);
  }, [search]);

  useEffect(() => {
    setStoredFilter(STORAGE_KEYS.CATEGORY, categoryId);
  }, [categoryId]);

  useEffect(() => {
    setStoredFilter(STORAGE_KEYS.STATUS, status);
  }, [status]);

  useEffect(() => {
    setStoredFilter(STORAGE_KEYS.PAGE, page);
  }, [page]);
